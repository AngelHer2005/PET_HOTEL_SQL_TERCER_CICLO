...
RETURNS TABLE
AS
	RETURN (SELECT F.ID_FACTURA CODIGO,F.SERVICIOS_AD NOMBRE_SERVICIOS,
	C.NOMBRE,C.APELLIDO,C.DNI,C.DIRECCIÓN
...
CREATE FUNCTION TRESERVA()
RETURNS TABLE
AS
	RETURN (SELECT R.ID_RESERVA CODIGO,R.PRECIO PRECIO_SERVICIOS,
	C.NOMBRE,C.EDAD,C.PESO,C.TAMAÑO
	FROM CANINO C JOIN RESERVA R ON C.ID_CANINO= R.ID_CANINO)
GO
SELECT * FROM TRESERVA() WHERE PRECIO_SERVICIOS = 110.00
GO
