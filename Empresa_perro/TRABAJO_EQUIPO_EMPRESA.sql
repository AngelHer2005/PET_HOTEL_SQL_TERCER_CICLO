...
PRINT 'NOMBREY APELLIDO: ' + @NOMBRE_APE
PRINT 'DNI: ' + @DNI
...
*/
CREATE FUNCTION THISTORIAL()
RETURNS TABLE
AS
	RETURN (SELECT H.ID_HISTORIAL CODIGO,H.DESCRIPCIÓN DESCRIPCION,R.PRECIO,R.FECHA_INI,R.FECHA_SAL
	
	FROM RESERVA R JOIN HISTORIAL H ON R.ID_RESERVA= H.ID_RESERVA)
GO
SELECT * FROM THISTORIAL() WHERE DESCRIPCION = 'Seguimiento de la medicación y cuidados especiales.'
...
AS
	RETURN (
		...
		JOIN VACUNA V ON P.ID_VACUNA = V.ID_VACUNA
	)
	SELECT * FROM VACUNADO() WHERE TIPO_DE_VACUNA='Leishmaniasis Canina'

/*
3. 
*/
CREATE FUNCTION CCANINO()
RETURNS TABLE
AS
    RETURN (
